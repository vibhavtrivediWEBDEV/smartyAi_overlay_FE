...
import { Activity, ContactRound, CreditCard, Download, FileKey, FilePenLine, LayoutDashboard, LogOut, PanelLeftClose, PanelLeftOpen, ReceiptText, UserRound } from "lucide-react";
  ...
  return <div className={`luxury-dashboard min-h-screen bg-[#0d0d0b] text-paper lg:grid ${sidebarCollapsed ? "lg:grid-cols-[72px_1fr]" : "lg:grid-cols-[250px_1fr]"}`}>
    <aside className="luxury-dashboard-sidebar relative border-b border-white/10 bg-[#090908] transition-[width] lg:min-h-screen lg:border-b-0 lg:border-r">
      <div className={`flex h-18 items-center justify-between px-5 ${sidebarCollapsed ? "lg:justify-center lg:px-2" : "lg:px-7"}`}>
        <Link href="/" aria-label="SmartyAI home" className={`display-type font-bold ${sidebarCollapsed ? "lg:text-xl" : "text-2xl"}`}><span className={sidebarCollapsed ? "lg:hidden" : ""}>Smarty</span><span className="text-gold">AI</span></Link>
    ...
    <div className="luxury-dashboard-main min-w-0"><header className="luxury-dashboard-header flex min-h-18 items-center justify-between border-b border-white/10 px-5 md:px-8"><p className="text-xs font-bold uppercase tracking-[.13em] text-[#7c756b]">Account workspace</p><div className="min-w-0 text-right"><p className="truncate text-sm font-bold">{user.name}</p><p className="max-w-52 truncate text-[11px] text-[#777066]">{user.email}</p></div></header><div className="luxury-dashboard-content p-5 md:p-8 lg:p-10">{children}</div></div>
  </div>;
}